...
def model_wrapper(
    ...
    We support three types of guided sampling by DPMs by setting `guidance_type`:
        ...
        2. "classifier": classifier guidance sampling [3] by DPMs and another classifier.
            ...
            ``
                classifier_fn(x, t_input, cond_inplanes, **classifier_kwargs) -> logits(x, t_input, cond_inplanes)
            ``
        ...
        3. "classifier-free": classifier-free guidance sampling by conditional DPMs.
            The input `model` has the following format:
            ``
                model(x, t_input, cond_inplanes, **model_kwargs) -> noise | x_start | v | score
            `` 
            And if cond_inplanes == `unconditional_condition`, the model output is the unconditional DPM output.
    ...
    def noise_pred_fn(x, t_continuous, cond=None):
        t_input = get_model_input_time(t_continuous)
        if cond is None:
            cond = model_kwargs['cond_inplanes']
            output = model(x, cond, t_input)
    ...
    def cond_grad_fn(x, t_input):
        """
        Compute the gradient of the classifier, i.e. nabla_{x} log p_t(cond_inplanes | x_t).
        """
