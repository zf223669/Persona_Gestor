...
from src.diffmotion.torchscale.architecture.utils import init_bert_params
from src.diffmotion.torchscale.component.droppath import DropPath
from src.diffmotion.torchscale.component.feedforward_network import FeedForwardNetwork, make_experts
from src.diffmotion.torchscale.component.multihead_attention import MultiheadAttention
from src.diffmotion.torchscale.component.relative_position_bias import RelativePositionBias
from src.diffmotion.torchscale.component.xmoe.moe_layer import MOELayer
from src.diffmotion.torchscale.component.xmoe.routing import Top1Gate, Top2Gate
...
class DecoderLayer(nn.Module):
    ...
    def forward(
        ...
        if self.mask_selection == 'causalMask':
            self_attn_mask = mask_strategy.causal_mask(batch_size, seq_length, self.causal_mask_diagonal)
        elif self.mask_selection == 'diagonal_matrix':
            print(f'upper_offset: {self.upper_offset}, lower_offset: {self.lower_offset}......')
            self_attn_mask = mask_strategy.upper_lower_diagonal_mask(batch_size, seq_length, self.upper_offset,
                                                                self.lower_offset)
