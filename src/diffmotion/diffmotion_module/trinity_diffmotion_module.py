...
class TrinityDiffmotionModule(LightningModule):
    ...
    def p_losses(self, x_start, cond, timestep, noise=None):
        ...
        model_out = self.model(inputs=x_t, cond=cond,
                               time=timestep, processing_state=self.process_state, last_time_stamp=self.num_timesteps)  # x_t: [B, 120, 65]  cond: [B, 120, 27]  time: [32]            # cond_inplanes [6080,1,45] t [6080]  model_out [6080,1,45]
    ...
    def p_mean_variance(self, x, cond, t, clip_denoised: bool):
        ...
        model_out = self.model(inputs=x, cond=cond, time=t, processing_state=self.process_state)  # [B, 400, 65]
    ...
    def training_step(self, batch, batch_idx):
        ...
        # load_data_time = self.trainer.profiler.recorded_durations["get_train_batch"][-1]
        # load_data_time = torch.tensor(load_data_time)
        # loss_dict.update({f'load_data_time': load_data_time})
    ...
    def validation_step(self, batch, batch_idx, *args: Any, **kwargs: Any):
        ...
        self.log_dict(loss_dict_no_ema, prog_bar=True, logger=True, on_step=False, on_epoch=True, sync_dist=self.log_sync_dist)
        self.log_dict(loss_dict_ema, prog_bar=True, logger=True, on_step=False, on_epoch=True, sync_dist=self.log_sync_dist)
    ...
    def test_step(self, batch: Any, batch_idx: int) -> Optional[STEP_OUTPUT]:
        ...
        for num in range(0, self.num_sequences):
            ...
            # folder_name = self.bvh_save_file + folder_name
            # bvh_save_name = os.path.join(path, folder_name)
    ...
    #     g_opt = torch.optim.Adam(self.motion_ae.parameters(), lr=1e-5)
    #     return g_opt
    def configure_optimizers(self):
