...
class TrinityGestureDataModule(LightningDataModule):
    def __init__(self,
        ...
        super(TrinityGestureDataModule, self).__init__()
    ...
    def prepare_data(self):
        ...
        # np.load(os.path.join(self.data_root, 'test_input_' + self.test_style + str(self.framerate) + 'fps.npz'))[
        #     'clips'].astype(np.float32)  # [3,400,27]
        # self.test_output = \
        #     np.load(os.path.join(self.data_root, 'test_output_' + str(self.framerate) + 'fps.npz'))[
        #         'clips'].astype(np.float32)  # [3,400,45]
        # log.info(
