...
class GestureDataModule(LightningDataModule):
    ...
    def setup(self, stage: Optional[str] = None) -> None:
        ...
        if stage in (None, "test")  or stage is None:
            ...
            for one_test_style in self.test_style:
                print(os.path.join(self.data_root,
                                 'test_input_' + one_test_style + '_' + str(self.framerate) + 'fps.npz'))
            self.test_inputs = [\
                np.load(
                    os.path.join(self.data_root,
                                 'test_input_' + one_test_style + '_' + str(self.framerate) + 'fps.npz'))[
                    'clips'].astype(np.float32) for one_test_style in self.test_style] # [3,400,27]]
            ...
            for test_input in self.test_inputs:
                if not self.test_dataset:
                    self.n_test = test_input.shape[0]  # 3
    ...
    def write_full_bvh(self, anim_clip, filename, paramValue,test_index):
