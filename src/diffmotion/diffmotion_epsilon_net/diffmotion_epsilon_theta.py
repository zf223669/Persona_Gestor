...
class FinalLayer(nn.Module):
    ...
    def __init__(self, hidden_size, out_channels, motion_decoder=None):
        ...
        # self.conv1d = nn.Conv1d(in_channels=hidden_size, out_channels=out_channels, kernel_size=1)
...
class TrinityEpsilonTheta(nn.Module):
    def __init__(self,
        ...
        if self.style_encode is True:
            log.info('Enable Style Encode!!!')
            if not conv_depthwise:
                self.style_encoder = nn.Sequential(
                    Transpose(shape=(1, 2)),
                    nn.Conv1d(in_channels=768, out_channels=768, kernel_size=500, stride=1, dilation=2),
                ...
                self.style_encoder = nn.Sequential(
                    nn.LayerNorm(768),
                    Transpose(shape=(1, 2)),
                    PointwiseConv1d(768, 768 * 2, stride=1, padding=0, bias=True),
                    GLU(dim=1),
                    DepthwiseConv1d(768, 768, 999, stride=1, padding=0, bias=False),
                    nn.BatchNorm1d(768),
                    Swish(),
                    PointwiseConv1d(768, 768, stride=1, padding=0, bias=True),
                    nn.Dropout(p=0.2),
                    Transpose(shape=(1, 2)),
                )  # the numbers of parameters in Conv1D
        ...
        self.down_sampler = nn.Sequential(
            ...
            nn.Conv1d(in_channels=768, out_channels=encoder_dim, kernel_size=201, stride=2, dilation=1),  # 20s
        ...
        if self.condition_strategy == 'concat_feature':
            self.concat_linear = nn.Linear(in_features=encoder_dim * 2, out_features=encoder_dim)
            self.concat_norm = nn.LayerNorm(encoder_dim, elementwise_affine=False, eps=1e-6)
    ...
    def forward(self, inputs, cond, time, processing_state='training', last_time_stamp=999):
        ...
        c = t + self.wav_encode
        if self.condition_strategy == 'concat_feature':
            x = self.concat_linear(torch.concatenate([x, c], dim=-1))
