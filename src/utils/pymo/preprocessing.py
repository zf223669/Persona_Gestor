...
from src.utils.pymo.rotation_tools import Rotation, euler2expmap, euler2expmap2, expmap2euler, euler_reorder, unroll
from src.utils.pymo.Quaternions import Quaternions
from src.utils.pymo.Pivots import Pivots
...
class JointSelector(BaseEstimator, TransformerMixin):
    ...
    def fit(self, X, y=None):
        ...
        print(f'selected Joint: {selected_joints}')
...
class Numpyfier(BaseEstimator, TransformerMixin):
    ...
    def inverse_transform(self, X, copy=None):
        ...
        for track in X:
            ...
            # smooth each 20 sec
            # track = Smooth_Gestures(track=track, threshold=3, window_length=5, polyorder=2)
...
def Smooth_Gestures(track, threshold, window_length, polyorder, filter_type='gaussian'):
    threshold = threshold
    total_smooth_step = track.shape[0] // 400 - 1
    for smooth_step in range(total_smooth_step):
        low_smooth_range = 400 * (smooth_step + 1) - threshold
        up_smooth_range = 400 * (smooth_step + 1) + threshold
        if filter_type == 'gaussian':
            track[low_smooth_range:up_smooth_range] = filters.gaussian_filter1d(track[low_smooth_range:up_smooth_range],
                                                                                window_length, axis=0, mode='nearest')
        elif filter_type == 'savgol':
            track[low_smooth_range:up_smooth_range] = savgol_filter(track[low_smooth_range:up_smooth_range],
                                                                    window_length=window_length,
                                                                    polyorder=polyorder, mode='nearest', axis=0)
        elif filter_type == 'mean':
            sub_array = track[low_smooth_range:up_smooth_range + 1]
            averages = (sub_array[:-1] + sub_array[1:]) / 2
            track[low_smooth_range:up_smooth_range] = averages
            # print(averages)
...
class RootTransformer(BaseEstimator, TransformerMixin):
    ...
    def transform(self, X, y=None):
        print(f"RootTransformer, method: {self.method}!!! !!!")
        Q = []
        ...
        for track in X:
            ...
            # end of abdolute_translation_deltas
                ...
                # forward_x = pd.Series(data=forward[:, 0], index=new_df.index)
                # forward_y = pd.Series(data=forward[:, 1], index=new_df.index)
                # forward_z = pd.Series(data=forward[:, 2], index=new_df.index)
                ...
                # f_z = 'f_z'
                #
                # new_df[f_x] = forward_x
                # new_df[f_y] = forward_y
                # new_df[f_z] = forward_z
                ...
                new_track.values = new_df
                new_track.values = new_track.values.loc[:,[dxp_col,dzp_col,dyr_col ]]
            #     new_track = track.clone()
            #
            #     # Absolute columns
            #     xp_col = '%s_Xposition' % track.root_name
            ...
            #     # rotations = np.pi / 180.0 * np.transpose(
            #     #     np.array([track.values[r1_col], track.values[r2_col], track.values[r3_col]]))
            #     new_df = track.values.copy().loc[:, [xp_col,yp_col,zp_col,xr_col,yr_col,zr_col]]
            #     # root_pos_x = pd.Series(data=positions[:, 0], index=new_df.index)
            #     # root_pos_y = pd.Series(data=positions[:, 1], index=new_df.index)
            #     # root_pos_z = pd.Series(data=positions[:, 2], index=new_df.index)
            #
    ...
    def inverse_transform(self, X, copy=None, start_pos=None):
        ...
        for track in X:
            ...
            # end of abdolute_translation_deltas
                ...
                # if 'f_x' in new_df.columns and 'f_y' in new_df.columns and 'f_z' in new_df.columns:
                #     f_x = 'f_x'
                #     f_y = 'f_y'
                #     f_z = 'f_z'
                #     new_df.drop([f_x, f_y, f_z], axis=1, inplace=True)
                new_track.values = new_df
...
class ConstantsRemover(BaseEstimator, TransformerMixin):
    ...
    def __init__(self, eps=1e-6):  # default: eps=1e-6
        self.eps = eps
