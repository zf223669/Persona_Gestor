...
from src.utils.pymo.data import MocapData
...
class BVHParser():
    ...
    def _parse_motion(self, bvh, start, stop):
        ...
        if stop > (len(bvh) - self.current_token) // len(self._motion_channels):
            stop = (len(bvh) - self.current_token) // len(self._motion_channels)
        ...
        # assert (1// frame_rate) == 120
        print(f'fps: {1 // frame_rate}')
        self.current_token = self.current_token + 1
        ...
        for i in tqdm(range(stop)):
            ...
            for channel in self._motion_channels:
                ...
                #     self.current_token = self.current_token + 1
                # except IndexError:
                #     print(f'len(bvh) = {len(bvh)}, self.current_token= {self.current_token}, bvh[self.current_token]=bvh[self.current_token]')
