...
def slice_data_with_waveform(motion_data, control_data, fps, audio_sample_rate, window_size, overlap):
    ...
    if n_sequences > 0:
        ...
        for i in range(0, n_sequences):
            ...
            sliced_control_data[i, :] = control_data[frameIdx // fps * audio_sample_rate:(frameIdx + window_size) // fps * audio_sample_rate].copy()
    else:
        print("WARNING: data too small for window")
...
def main(cfg: DictConfig) -> Optional[float]:
    ...
    if extract_MFCC:
        ...
        for test_file in held_out:
            ...
            test_motion, test_ctrl = import_and_slice_with_waveform(temp_test_file_list, motion_path, speech_path, slice_win_test, 0,
    ...
    if is_standardize:
        ...
        if not extract_MFCC:    # Raw MFCC
            ...
            for test_file in held_out:
                ...
                test_motion, test_ctrl = import_and_slice(temp_test_file_list, motion_path, speech_path, slice_win_test, 0, mirror=False,
                                                          start=0)
